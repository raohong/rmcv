...
const IndexBar = React.forwardRef<HTMLDivElement, IndexBarProps>((_props, ref) => {
  const {
    className,
    stickyOffsetTop,
    onSelect,
    ...
    ...props
  } = useThemeProps(IndexBarName, _props);
  ...
  const componentState: IndexBarComponentState = useMemo(
    ...
    [sticky, zIndex, highlightColor, palette.primary],
  );
  ...
  useDrag(
    ({ memo, initial, xy, tap }) => {
      ...
      if (current && current.index !== indexValueRef.current) {
        ...
        onChangeCallback?.(current.index);
  ...
  }, [scrollableParent, indexValueRef, anchors, setCurrentIndex, onChangeCallback]);
